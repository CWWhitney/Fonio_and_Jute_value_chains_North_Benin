...
# Extract priority weights (first row) and category definitions
priority_weights <- unified_matrix %>%
  filter(type == "weight") %>%
  select(-type, -objective, -category) %>%
  pivot_longer(everything(), names_to = "intervention", values_to = "weight")

category_definitions <- unified_matrix %>%
  filter(type == "score") %>%
  select(objective, category)

# Extract intervention scores (remaining rows)
score_matrix <- unified_matrix %>%
  filter(type == "score") %>%
  select(-type, -category)

# Calculate weighted scores
# We split all objectives into two buckets:
# Impact - nutrition, livelihoods, environment, equity, etc.
# Feasibility - feasibility, lower_market_risk, cost, etc.
results <- score_matrix %>%
  pivot_longer(cols = -objective, names_to = "intervention", values_to = "impact_score") %>%
  left_join(priority_weights, by = "intervention") %>%
  left_join(category_definitions, by = "objective") %>%
  # Weighted scores
  mutate(weighted_impact = impact_score * weight/100) %>% 
  group_by(intervention, category) %>%
  summarise(
    # Aggregate weighted sums
    category_score = sum(weighted_impact),
    .groups = "drop"
  ) %>%
  pivot_wider(names_from = category, values_from = category_score) %>%
  # add impact and feasibility into total_score for bubble size
  # Composite metric
  mutate(total_score = impact + feasibility) %>%
  arrange(desc(total_score))

# Prepare data for the complete matrix visualization (transposed version)
...
# Get all intervention columns dynamically (exclude metadata columns)
...
ggplot(heatmap_display_data, aes(x = intervention, y = label, fill = score)) +
  geom_tile(color = "white", size = 1) +
  geom_text(aes(label = score), color = "black", size = 4) +
  scale_fill_gradient2(low = "white", high = "green", 
                       midpoint = 50, name = "Score") +
  labs(title = "Intervention Impact Matrix",
       subtitle = "Scores show expected impact (0-100) on each objective",
...
ggplot(results, aes(x = feasibility, y = impact, size = total_score)) +
  ...
  annotate("text", x = 80, y = 20, label = "Easy Wins", color = "red", fontface = "bold") +
  annotate("text", x = 80, y = 80, label = "Strategic Priorities", color = "darkgreen", fontface = "bold") +
  annotate("text", x = 20, y = 80, label = "High Effort", color = "orange", fontface = "bold") +
  annotate("text", x = 20, y = 20, label = "Avoid", color = "grey", fontface = "bold") +
